...
@interface FTCalendarCell : UICollectionViewCell
...
//        [_dayLabel addGestureRecognizer:labelTapGestureRecognizer];
...
typedef NS_ENUM(NSUInteger, FTCalendarMonth) {
    FTCalendarMonthPrevious = 0,
    FTCalendarMonthCurrent = 1,
    FTCalendarMonthNext = 2,
};

@interface FTCalendarItem () <UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>

@property (strong, nonatomic) UICollectionView *collectionView;

@end

@implementation FTCalendarItem
...
- (void)setupCollectionView {
    ...
    [self.collectionView registerClass:[FTCalendarCell class] forCellWithReuseIdentifier:@"CalendarCell"];
}
...
// 获取某月day的日期
- (NSDate *)dateOfMonth:(FTCalendarMonth)calendarMonth WithDay:(NSInteger)day {
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDate *date;
    
    switch (calendarMonth) {
        case FTCalendarMonthPrevious:
            date = [self previousMonthDate];
            break;
            
        case FTCalendarMonthCurrent:
            date = self.date;
            break;
            
        case FTCalendarMonthNext:
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FTCalendarCell *cell = [self.collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
    ...
    if (indexPath.row < firstWeekday) {    // 小于这个月的第一天
        ...
        cell.chineseDayLabel.text = [self chineseCalendarOfDate:[self dateOfMonth:FTCalendarMonthPrevious WithDay:day]];
        ...
        cell.dayLabel.text = [NSString stringWithFormat:@"%ld", (long)day];
        cell.dayLabel.textColor = [UIColor grayColor];
        cell.chineseDayLabel.text = [self chineseCalendarOfDate:[self dateOfMonth:FTCalendarMonthNext WithDay:day]];
    } else {    // 属于这个月
        ...
        if ([[NSCalendar currentCalendar] isDate:[NSDate date] equalToDate:self.date toUnitGranularity:NSCalendarUnitMonth] && ![[NSCalendar currentCalendar] isDateInToday:self.date]) {
            ...
            if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:[NSDate date]]) {
                ...
                cell.chineseDayLabel.textColor=[UIColor redColor];
            }
        }
        
        cell.chineseDayLabel.text = [self chineseCalendarOfDate:[self dateOfMonth:FTCalendarMonthCurrent WithDay:day]];
    }
